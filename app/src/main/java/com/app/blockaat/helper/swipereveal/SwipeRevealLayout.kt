package com.app.blockaat.helper.swipereveal
        ...
        for (index in 0 until childCount) {
            ...
            when (dragEdge) {
                ...
                DRAG_EDGE_BOTTOM -> {
                    left = Math.min(paddingLeft, maxRight)
                    top =
                        Math.max(b - measuredChildHeight - paddingBottom - t, minTop)
                    right = Math.min(measuredChildWidth + paddingLeft, maxRight)
...
/* @Override
    ...
    override fun onMeasure(
        ...
        for (i in 0 until childCount) {
            ...
            if (childParams.height == LayoutParams.WRAP_CONTENT &&
                ...
                Math.max(child.measuredHeight, wrapChildHeight) else desiredHeight =
                Math.max(child.measuredHeight, desiredHeight)
            if (childParams.width == LayoutParams.WRAP_CONTENT &&
                (dragEdge == DRAG_EDGE_TOP || dragEdge == DRAG_EDGE_BOTTOM)
            ) wrapChildWidth =
                Math.max(child.measuredWidth, wrapChildWidth) else desiredWidth =
                Math.max(child.measuredWidth, desiredWidth)
        }
    ...
    private fun init(
        ...
        if (attrs != null && context != null) {
            ...
            mMode = a.getInteger(
                R.styleable.SwipeRevealLayout_mode,
                MODE_NORMAL
            )
...
// properly on RecyclerView or view that handles scroll gesture.
    ...
    private val mDragHelperCallback: ViewDragHelper.Callback = object : ViewDragHelper.Callback() {
        ...
        override fun onEdgeDragStarted(
            edgeFlags: Int,
            pointerId: Int
        ) {
